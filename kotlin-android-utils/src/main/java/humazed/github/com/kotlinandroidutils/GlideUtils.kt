...
fun ImageView.load(url: String?, options: RequestOptions.() -> Unit = {}) =
    customGlide(context, options).also {
        if (!url.isNullOrBlank()) it.load(url).into(this)
    }

fun ImageView.load(url: Uri?, options: RequestOptions.() -> Unit = {}) =
    customGlide(context, options).also {
        if (url != null) it.load(url).into(this)
    }

fun ImageView.load(file: File?, options: RequestOptions.() -> Unit = {}) =
    customGlide(context, options).also {

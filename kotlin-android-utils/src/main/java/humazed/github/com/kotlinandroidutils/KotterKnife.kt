...
fun Context.bindBool(@BoolRes id: Int): ReadOnlyProperty<Context, Boolean> = requiredRes(id, boolFinder)
fun Context.bindColor(@ColorRes id: Int): ReadOnlyProperty<Context, Int> = requiredRes(id, boolFinder)
...
private val Context.boolFinder: Context.(Int) -> Boolean?
    get() = { resources.getBoolean(it) }
private val Context.colorFinder: Context.(Int) -> Int?
...
private fun resNotFound(id: Int, desc: KProperty<*>): Nothing = throw IllegalStateException("Resource ID $id for '${desc.name}' not found.")
...
@Suppress("UNCHECKED_CAST")
private fun <T, V> requiredRes(id: Int, finder: T.(Int) -> Boolean?)
        = Lazy { t: T, desc -> t.finder(id) as V ?: resNotFound(id, desc) }

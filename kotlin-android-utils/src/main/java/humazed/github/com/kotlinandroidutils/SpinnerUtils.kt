...
fun <T> AutoCompleteTextView.onItemSelected(items: List<T>, names: List<String>, onItemSelected: (item: T) -> Unit) {
    onItemSelected(items, names) { item, _ -> onItemSelected(item) }
}


fun <T> AutoCompleteTextView.onItemSelected(items: List<T>, mapNames: (T) -> String, onItemSelected: (item: T, position: Int) -> Unit) {
    onItemSelected(items, items.map(mapNames), onItemSelected)
}

fun <T> AutoCompleteTextView.onItemSelected(items: List<T>, mapNames: (T) -> String, onItemSelected: (item: T) -> Unit) {
